...
def capsBoard(page):
    ...
    # CapsKey = createButton(row4, "Caps Lock", width=12, link="capsBoard")
...
def createButton(row, val, other="", label="", width=6, height=3, link="", page=""):
    if label == "":
        label = val
    if link == "capsBoard":
        button = Button(row, text=other+"\n"+label, width=width, height=height, borderwidth=1,bg=bg, fg=fg, anchor=N, command=lambda page=page: capsBoard(page))
    else:
...
def mainBoard():
    ...
    CapsKey = createButton(row4, "Caps Lock", width=12, link="capsBoard", page=home)
    AKey = createButton(row4, "A")
    SKey = createButton(row4, "S")
    DKey = createButton(row4, "D")
    FKey = createButton(row4, "F")
